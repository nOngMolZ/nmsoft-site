import * as THREE from "three";

const HeroLights = () => {
  return (
    <>
      {/* Remove existing lights */}
      {/* <spotLight position={[2, 5, 6]} intensity={100} angle={0.15} penumbra={0.2} color="white" />
      <spotLight position={[4, 5, 4]} intensity={40} angle={0.3} penumbra={0.5} color="#4cc9f0" />
      <spotLight position={[-3, 5, 5]} intensity={60} angle={0.4} penumbra={1} color="#9d4edd" />
      <primitive object={new THREE.RectAreaLight("#A259FF", 8, 3, 2)} position={[1, 3, 4]} intensity={15} rotation={[-Math.PI / 4, Math.PI / 4, 0]} />
      <pointLight position={[0, 1, 0]} intensity={10} color="#7209b7" />
      <pointLight position={[1, 2, -2]} intensity={10} color="#0d00a4" /> */}

      {/* Add daylight simulation */}
      <ambientLight intensity={0.8} color="#ffffff" />
      <directionalLight
        position={[10, 15, 5]} // Position the sun high and slightly angled
        intensity={2.5}
        color="#ffffff" // Slightly warm sun color
        castShadow // Enable shadows
        shadow-mapSize-width={2048} // Increase shadow map resolution for better quality
        shadow-mapSize-height={2048}
        shadow-camera-far={50}
        shadow-camera-left={-10}
        shadow-camera-right={10}
        shadow-camera-top={10}
        shadow-camera-bottom={-10}
      />
    </>
  );
};

export default HeroLights;
